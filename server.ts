...
import { createUniversityRouter } from "./server/university/universityRoutes.js";
import { UNIVERSITY_TENANT_CLASSIFICATIONS, universityStore, type UniversityTenantClassification } from "./server/university/universityStore.js";
...
app.post("/api/intake/:id/promote", (req, res) => {
  try {
    ...
    if (result.intakeSession.classification === 'ACADEMY') universityStore.linkWorkspace(result.intakeSession.id, result.workspace.id);
    saveStorage();
...
app.use("/api/university", createUniversityRouter());
...
async function startServer() {
  ...
  if (process.env.IS_SCRIPT !== "true") {
    app.listen(PORT, "0.0.0.0", async () => {
      console.log(`Eve's Bookkeeping API ${apiOnly ? "(API_ONLY) " : ""}running on http://localhost:${PORT}`);
