...
async function executeWorkerExtraction(job: WorkerJob) {
  ...
  try {
    ...
    // Preserve model/parser facts, but do not let a partial model response hide
    // other financial statements that are physically present in parsed tables.
    // This matters for full issuer filings where a bounded model extraction may
    // return the income statement while the same hash-verified source also
    // contains a balance sheet. Deterministic additions retain their own source
    // coordinates and are deduplicated against the model result.
    const parsedFacts = parsedDoc.facts && Array.isArray(parsedDoc.facts) ? parsedDoc.facts : [];
    const deterministicFacts = extractDeterministicFactsFromDocument(parsedDoc, job);
    facts.push(...mergeComplementaryExtractedFacts(parsedFacts, deterministicFacts));
...
function factSemanticKey(fact: any): string {
  const metric = String(fact?.canonicalMetric || fact?.labelNormalized || fact?.labelOriginal || '').toLowerCase().replace(/[^a-z0-9]/g, '');
  const period = String(fact?.reportingPeriod || fact?.periodEnd || '').trim();
  const value = String(fact?.normalizedValue ?? fact?.valueFunctional ?? '').trim();
  return `${metric}|${period}|${value}`;
}

export function mergeComplementaryExtractedFacts(parsedFacts: ExtractedFact[], deterministicFacts: ExtractedFact[]): ExtractedFact[] {
  const merged = [...(parsedFacts || [])];
  const seen = new Set(merged.map(factSemanticKey));
...
export function extractDeterministicFactsFromDocument(parsedDoc: any, job: WorkerJob): ExtractedFact[] {
            ...
            for (let cIdx = 1; cIdx < row.length; cIdx++) {
              ...
              if (!isNaN(parsedNum) && cleanNumberStr.length > 0) {
                ...
                extractedFacts.push({
                  ...
                  sourceSha256: job.documentHash,
                  sourceExtractionMethod: valueEvidence?.coordinate?.extractionMethod || "HTML_NATIVE_TABLE",
                  sourceExtractionVersion: valueEvidence?.coordinate?.extractionVersion || "anydoc-2.1",
        ...
        if (m && m[1]) {
          ...
          if (num > 0) {
            ...
            extractedFacts.push({
              ...
              sourceSha256: job.documentHash,
              sourceExtractionMethod: ocrEvidence?.coordinate?.extractionMethod || "DETERMINISTIC_NATIVE_TEXT",
              sourceExtractionVersion: ocrEvidence?.coordinate?.extractionVersion || "1",
