...
export function reviewRow(f: any): any {
  ...
  return { id: f.id || null, metric: f.canonicalMetric, label: f.label || f.canonicalMetric,
    ...
    factState: f.factState || f.status || 'NOT_RECORDED', unitScale: f.unitScale || 'NOT_RECORDED',
    normalizedScaleMultiplier: f.normalizedScaleMultiplier ?? null };
}
...
export function buildReviewCsv(facts: any[], currency: string, apReview?: any, bankReview?: any, trialBalanceReview?: any, mixedSourceReview?: any, mixedSourceBatchReview?: any, duplicateEvidenceReview?: any, packageIdentity?: any, adjudicationLineage?: any): string {
  const rows = [['Source Fact ID','Metric','Label','Value','Currency','Unit Scale','Scale Multiplier','Reporting Period','Statement','Document ID','Source Document','Extractor Locator','Fact State','Verification','Evidence Status','Source Block IDs','Source SHA256','Source Provenance IDs','Source Coordinate','Extraction Method','Extraction Version','Confidence','Source Excerpt','Derivation ID','Derivation Formula','Derivation Operation','Operand Fact IDs','Operand Values'],
    ...facts.map(f=>{const r=reviewRow(f);return [r.id,r.metric,r.label,r.value,currency,r.unitScale,r.normalizedScaleMultiplier,r.period,r.statement,r.documentId,r.sourceDoc,r.extractorPage,r.factState,r.verificationStatus,r.evidenceStatus,r.sourceBlockIds.join(';'),r.sourceSha256,r.sourceProvenanceIds.join(';'),sourceCoordinateText(r),r.sourceExtractionMethod,r.sourceExtractionVersion,r.sourceConfidence,r.sourceText,r.derivationId,r.derivationFormula,r.derivationOperation,r.operandFactIds.join(';'),r.operandValues?JSON.stringify(r.operandValues):''];})];
  const identityRows = packageIdentity ? [
    ['DELIVERABLE PACKAGE IDENTITY'],
    ['Report ID', packageIdentity.reportId],
    ['Version', packageIdentity.version],
    ['Engagement ID', packageIdentity.engagementId],
    ['Workspace ID', packageIdentity.workspaceId],
    ['Client', packageIdentity.clientName],
    ['Period', packageIdentity.period],
    ['Currency', currency],
    []
  ] : [];
  const sections = [[...identityRows, ...rows].map(row=>row.map(csvCell).join(',')).join('\r\n')];
  if (apReview) {
    const a=apReview;
  ...
  if (duplicateEvidenceReview) {
    const d=duplicateEvidenceReview;const dupRows:any[][]=[[],['DUPLICATE EVIDENCE INTEGRITY REVIEW'],['Status',d.status],['Promotion State',d.promotionState],['Action',d.decision],['Input Observations',d.inputObservationCount],['Unique Physical Sources',d.uniquePhysicalSourceCount],['Evidentiary Identities',d.evidenceIdentityCount],['Independent Corroboration',d.independentCorroborationCount],['Suppressed Exact Duplicates',d.suppressedExactDuplicateCount],['Suppressed Cosmetic Near Duplicates',d.suppressedCosmeticNearDuplicateCount],['Material Conflicts',d.materialConflictCount],['Canonical Promoted Value',d.canonicalPromotedValue??'NOT PROMOTED'],['Issues',(d.issues||[]).join(' | ')],[],['Source Kind','Classification','Document ID','Total','Source SHA256','Identity Fingerprint','Material Fingerprint','Representative','Corroboration Weight','Arithmetic','Coordinate','Provenance'],...(d.variants||[]).map((v:any)=>[v.sourceKind,v.classification,v.documentId,v.materialFields?.total,v.sourceSha256,v.identityFingerprint,v.materialFingerprint,v.representativeDocumentId,v.corroborationWeight,v.arithmeticStatus,sourceCoordinateText({sourceCoordinate:v.sourceCoordinate}),v.provenanceId])];
    sections.push(dupRows.map(row=>row.map(csvCell).join(',')).join('\r\n'));
  }
  if (adjudicationLineage) {
    const a=adjudicationLineage;
    const adjudicationRows:any[][]=[[],['CONFLICT CLARIFICATION AND RESOLUTION LINEAGE'],['Conflict ID',a.conflictId],['Gap ID',a.gapId],['Clarification/PBC ID',a.clarificationId],['Affected Conclusion',a.affectedConclusionId],['Final Status',a.finalStatus],['Clarification Response',a.clarificationResponse],['Response Alone Cleared','NO'],['Final Decision ID',a.finalDecisionId],['Final Decision Hash',a.finalDecisionHash],[],['Evidence Role','Document ID','Source SHA256','Provenance IDs','Source Block IDs','Locator'],...(a.evidence||[]).map((e:any)=>[e.role,e.documentId,e.sourceSha256,(e.provenanceIds||[]).join(';'),(e.sourceBlockIds||[]).join(';'),e.locator||'NOT_RECORDED'])];
    sections.push(adjudicationRows.map(row=>row.map(csvCell).join(',')).join('\r\n'));
  }
...
export async function renderReviewPdf(params: any, dir: string) {
  ...
  text(`Engagement: ${params.engagementId || 'NOT_RECORDED'} | Workspace: ${params.workspaceId || 'NOT_RECORDED'}`);
  ...
  if(params.mixedSourceBatchReview){const b=params.mixedSourceBatchReview;heading('Mixed-source batch isolation review');text(`Status: ${b.status}\nPromotion state: ${b.promotionState}\nAction: ${b.decision}\nSource families: ${b.sourceFamilyCount} / 4\nUnique source identities: ${b.uniqueSourceCount}\nCanonical batch value: ${b.canonicalBatchValue??'NOT AGGREGATED'}`);text('Similar values remain independent source-scoped observations; source identity is established by document, SHA, artifact, provenance and coordinate family.');for(const i of b.sourceItems||[]){text(`${i.sourceKind} | ${i.semanticRole} | ${i.currency} ${i.value}`,9,true);text(`Document: ${i.documentId}\nSource SHA-256: ${i.sourceSha256}\nCoordinate SHA-256: ${i.coordinateSourceSha256}\nProvenance: ${i.provenanceId}\nSource artifact: ${i.sourceArtifactId}\nCoordinate artifact: ${i.coordinateSourceArtifactId}\nCoordinate: ${sourceCoordinateText({sourceCoordinate:i.sourceCoordinate})}\nIssues: ${(i.issues||[]).join(' | ')||'NONE'}`);}for(const issue of b.issues||[])text(`Batch source-identity issue: ${issue}`);}
  if(params.duplicateEvidenceReview){const d=params.duplicateEvidenceReview;heading('Duplicate / near-duplicate evidence integrity review');text(`Status: ${d.status}\nPromotion state: ${d.promotionState}\nAction: ${d.decision}\nInputs: ${d.inputObservationCount}\nUnique physical sources: ${d.uniquePhysicalSourceCount}\nEvidentiary identities: ${d.evidenceIdentityCount}\nIndependent corroboration: ${d.independentCorroborationCount}\nSuppressed exact duplicates: ${d.suppressedExactDuplicateCount}\nSuppressed cosmetic near-duplicates: ${d.suppressedCosmeticNearDuplicateCount}\nMaterial conflicts: ${d.materialConflictCount}\nCanonical promoted value: ${d.canonicalPromotedValue??'NOT PROMOTED'}`);text('Byte-identical and cosmetic-only copies do not increase corroboration weight. Same-identity evidence with changed material accounting content remains separately traceable and blocks promotion.');for(const v of d.variants||[]){text(`${v.sourceKind} | ${v.classification} | USD ${v.materialFields?.total}`,9,true);text(`Document: ${v.documentId}\nSource SHA-256: ${v.sourceSha256}\nIdentity fingerprint: ${v.identityFingerprint}\nMaterial fingerprint: ${v.materialFingerprint}\nRepresentative: ${v.representativeDocumentId||'NONE'}\nCorroboration weight: ${v.corroborationWeight}\nArithmetic: ${v.arithmeticStatus}\nCoordinate: ${sourceCoordinateText({sourceCoordinate:v.sourceCoordinate})}\nProvenance: ${v.provenanceId}`);}for(const issue of d.issues||[])text(`Duplicate evidence issue: ${issue}`);}
  if(params.adjudicationLineage){const a=params.adjudicationLineage;heading('Conflict, clarification and resolution lineage');text(`Conflict: ${a.conflictId}\nGap: ${a.gapId}\nClarification/PBC: ${a.clarificationId}\nAffected conclusion: ${a.affectedConclusionId}\nFinal status: ${a.finalStatus}\nFinal decision: ${a.finalDecisionId}\nDecision hash: ${a.finalDecisionHash}`);text(`Clarification response: ${a.clarificationResponse}\nResponse alone cleared the gap: NO`);for(const e of a.evidence||[])text(`${e.role} | Document: ${e.documentId}\nSource SHA-256: ${e.sourceSha256}\nProvenance: ${(e.provenanceIds||[]).join(', ')||'NOT_RECORDED'}\nSource blocks: ${(e.sourceBlockIds||[]).join(', ')||'NOT_RECORDED'}\nLocator: ${e.locator||'NOT_RECORDED'}`);}
  ...
  facts.forEach((f:any,i:number)=>{const r=reviewRow(f);if(y<130)newPage();text(`${i+1}. ${r.label}`,10,true);
    ...
    text(`Fact state: ${r.factState} | Checks: ${r.verificationStatus} / ${r.evidenceStatus} | Scale: ${r.unitScale} (x${r.normalizedScaleMultiplier ?? 'NOT_RECORDED'})`);
    text(`Source SHA-256: ${r.sourceSha256 || 'NOT_RECORDED'}\nSource block IDs: ${r.sourceBlockIds.length ? r.sourceBlockIds.join(', ') : 'NOT_RECORDED'}\nSource provenance IDs: ${r.sourceProvenanceIds.length ? r.sourceProvenanceIds.join(', ') : 'NOT_RECORDED'}\nSource coordinate: ${sourceCoordinateText(r)}`);
...
export function renderReviewWorkbook(params:any, dir:string){
  ...
  const summary=add('Executive Summary',[
    ...
    ['Engagement ID',params.engagementId||'NOT_RECORDED'],['Workspace ID',params.workspaceId||'NOT_RECORDED'],
  ...
  if(params.balanceIdentityApplicable){
    summary.B17={t:'n',f:'B14-B15-B16',v:b.assets-b.liabilities-b.equity};
    for(const addr of ['B14','B15','B16','B17'])if(summary[addr])summary[addr].z='#,##0.00;[Red](#,##0.00);"-"';
  }
  const rows=params.facts.map((f:any)=>reviewRow(f));
  const financial=add('Financial Statements',[
    ['Source Fact ID','Metric / Label','Value','Currency','Unit Scale','Scale Multiplier','Reporting Period','Statement','Fact State','Verification','Evidence'],
    ...rows.map((r:any)=>[r.id,r.label,r.value,params.currency,r.unitScale,r.normalizedScaleMultiplier,r.period,r.statement,r.factState,r.verificationStatus,r.evidenceStatus])
  ],[48,44,22,12,18,18,28,36,20,20,20]);
  ...
  if(params.mixedSourceBatchReview){const b=params.mixedSourceBatchReview;add('Mixed Source Batch',[[ 'Field','Recorded value'],['Status',b.status],['Promotion State',b.promotionState],['Action',b.decision],['Canonical Batch Value',b.canonicalBatchValue??'NOT AGGREGATED'],['Source Family Count',b.sourceFamilyCount],['Unique Source Count',b.uniqueSourceCount],['Batch Issues',(b.issues||[]).join(' | ')],[],['Source Kind','Semantic Role','Value','Currency','Document ID','Source SHA256','Coordinate SHA256','Provenance','Source Artifact','Coordinate Artifact','Coordinate','Issues'],...(b.sourceItems||[]).map((i:any)=>[i.sourceKind,i.semanticRole,i.value,i.currency,i.documentId,i.sourceSha256,i.coordinateSourceSha256,i.provenanceId,i.sourceArtifactId,i.coordinateSourceArtifactId,sourceCoordinateText({sourceCoordinate:i.sourceCoordinate}),(i.issues||[]).join(' | ')])],[22,32,14,12,28,66,66,60,52,52,86,76]);}
  if(params.duplicateEvidenceReview){const d=params.duplicateEvidenceReview;add('Duplicate Evidence',[[ 'Field','Recorded value'],['Status',d.status],['Promotion State',d.promotionState],['Action',d.decision],['Input Observations',d.inputObservationCount],['Unique Physical Sources',d.uniquePhysicalSourceCount],['Evidentiary Identities',d.evidenceIdentityCount],['Independent Corroboration',d.independentCorroborationCount],['Suppressed Exact Duplicates',d.suppressedExactDuplicateCount],['Suppressed Cosmetic Near Duplicates',d.suppressedCosmeticNearDuplicateCount],['Material Conflicts',d.materialConflictCount],['Canonical Promoted Value',d.canonicalPromotedValue??'NOT PROMOTED'],['Issues',(d.issues||[]).join(' | ')],[],['Source Kind','Classification','Document ID','Total','Source SHA256','Identity Fingerprint','Material Fingerprint','Representative','Corroboration Weight','Arithmetic','Coordinate','Provenance'],...(d.variants||[]).map((v:any)=>[v.sourceKind,v.classification,v.documentId,v.materialFields?.total,v.sourceSha256,v.identityFingerprint,v.materialFingerprint,v.representativeDocumentId,v.corroborationWeight,v.arithmeticStatus,sourceCoordinateText({sourceCoordinate:v.sourceCoordinate}),v.provenanceId])],[18,38,30,14,66,66,66,30,20,16,82,52]);}
  if(params.adjudicationLineage){const a=params.adjudicationLineage;add('Adjudication Lineage',[[ 'Field','Recorded value'],['Conflict ID',a.conflictId],['Gap ID',a.gapId],['Clarification/PBC ID',a.clarificationId],['Affected Conclusion',a.affectedConclusionId],['Final Status',a.finalStatus],['Clarification Response',a.clarificationResponse],['Response Alone Cleared','NO'],['Final Decision ID',a.finalDecisionId],['Final Decision Hash',a.finalDecisionHash],[],['Evidence Role','Document ID','Source SHA256','Provenance IDs','Source Block IDs','Locator'],...(a.evidence||[]).map((e:any)=>[e.role,e.documentId,e.sourceSha256,(e.provenanceIds||[]).join(';'),(e.sourceBlockIds||[]).join(';'),e.locator||'NOT_RECORDED'])],[30,48,68,72,72,90]);}
