...
import { Plus, Search, ArrowUpRight, ExternalLink } from 'lucide-react';
...
export const ProjectsView: React.FC<ProjectsViewProps> = ({
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200 font-mono">
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-2xs flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Engagements are created when an intake session is promoted to a workspace.
          </p>
        </div>
      ...
      {filtered.length === 0 ? (
        <EmptyExtractionState title="No engagements" onUpload={onOpenUpload} />
      ) : (
          ...
          {filtered.map((proj) => (
            ...
            >
              <div className="flex items-start justify-between">
                <div>
                  <span className="text-[10px] font-bold uppercase text-slate-400">{proj.companyName}</span>
              ...
              <div className="space-y-2 text-xs text-slate-600 bg-slate-50 p-3 rounded-xl border border-slate-100">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-blue-600">{proj.facts || EMPTY_DISPLAY}</span>
                </div>
                <div className="flex justify-between">
                  <span>Documents Parsed:</span>
                  <span className="font-bold text-slate-900">{proj.docsCount || EMPTY_DISPLAY}</span>
                </div>
                <div className="flex justify-between">
                  <span>Lead CPA:</span>
