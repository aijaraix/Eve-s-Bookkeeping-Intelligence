...
import { Building2, Search, Plus, ExternalLink, ShieldCheck, ArrowUpRight } from 'lucide-react';
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="px-2.5 py-0.5 rounded-full text-xs font-bold bg-blue-50 text-blue-700 border border-blue-200">
              {COMPANIES.length} Corporate Entities Monitored
            </span>
          </div>
          <p className="text-xs text-slate-500 mt-1">
            Global Enterprise Client Directory & Accounting Standard Directory • Click any company to open its financial workspace
          </p>
        </div>
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-400 mt-0.5">{c.country} • {c.reporting} ({c.ticker})</p>
                  </div>
                </div>
              </div>

              <div className="text-xs text-slate-500 space-y-1 bg-slate-50 p-3 rounded-xl border border-slate-100">
                <div>Registration: <span className="font-bold text-slate-700">{c.reg}</span></div>
                <div>Currency: <span className="font-bold text-slate-700">{c.currency}</span></div>
