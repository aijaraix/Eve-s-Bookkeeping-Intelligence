...
export class OCRParser {
  ...
  public async parse(
    ...
    const requestedPdfPageNumbers: number[] | undefined = isPdf && Array.isArray(inspection?.ocrPageNumbers)
      ? Array.from(new Set<number>(inspection.ocrPageNumbers.map(Number).filter((n: number) => Number.isInteger(n) && n > 0))).sort((a, b) => a - b)
      : undefined;
